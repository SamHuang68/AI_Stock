#!/usr/bin/env python3
"""Wrapper #3 to defeat sandbox mount cache — same logic as build_v2.py."""
...
# 預設 chart range '5y' -> '1d'
RNG_OLD = "range: '5y',                 // iOS-style chart range: 1d/3w/1mo/3mo/6mo/ytd/1y/2y/5y/10y/max"
RNG_NEW = "range: '6mo',                // iOS-style chart range: 1d/3w/1mo/3mo/6mo/ytd/1y/2y/5y/10y/max  (v3.2 預設 6 個月日 K)"
if RNG_OLD in html:
...
print('[OK] wrote', DST, '(', len(html), 'bytes ) ts=', ts)
print('     modules:', ', '.join(V2_SCRIPTS))
