...
def build_report_html(delta):
    ...
    # 5. 各 ETF 明細
    H.append('<h3 style="color:#e2e8f0;border-bottom:1px solid #334155;padding-bottom:4px">5. 各 ETF 明細</h3>')
    for e in etfs:
        nn, nr, nc = len(e.get('new') or []), len(e.get('removed') or []), len(e.get('changed') or [])
        H.append(f'<div style="margin:14px 0 4px;font-weight:700;color:#fbbf24">{e.get("code")} {e.get("name","")}'
                 f' <span style="color:#64748b;font-weight:400;font-size:11px">總持股 {e.get("total","-")}｜新增 {nn}｜刪除 {nr}｜加減碼 {nc}</span></div>')
        # 新增
        nrows = [[x.get('rank'), x.get('code'), x.get('name', ''), f"{x.get('weight',0):.2f}%",
                  _n(x.get('shares')), _priority(x.get('rank')), _layout(x.get('weight'))] for x in (e.get('new') or [])]
        if nrows:
        ...
        # 加減碼
        crows = []
        for x in sorted(e.get('changed') or [], key=lambda y: abs(y.get('shares_delta') or 0), reverse=True):
