...
ENV_KEYS = {
    ...
    'shadowTouxin5d': 'ST_SHADOW_TOUXIN_5D',
...
def feature_flags(base_dir: str | Path | None = None) -> dict[str, bool]:
    ...
    for key in FEATURE_KEYS:
        ...
        if key in local:
            ...
            out[key] = True
        elif key in ('ohlcLedger', 'shadowPeakObservation', 'shadowPeak100d', 'shadowTouxin5d') and os.environ.get(env_name) is not None:
            out[key] = False
        elif master:
            out[key] = True
        elif key in ('ohlcLedger', 'shadowPeakObservation', 'shadowPeak100d', 'shadowTouxin5d'):
