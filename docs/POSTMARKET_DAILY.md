...
| `POST` | `/api/ai/postmarket-daily/abort` | `{abortSignalClientId}` 中止批次（呼叫前生效） |
...
- system：`postmarket_report.SYSTEM_PROMPT` 逐字採用 Research 提供的 canonical 字串
...
- Call hints：`temperature 0.2`（0–0.3）；`max_tokens` 隨檔數縮放
