...
(function () {
  ...
  function ensureStyle() {
    ...
    s.textContent =
      ...
      '.vz-spark-wrap{position:relative;width:100%;min-height:28px;overflow:visible}' +
      '.vz-spark-wrap > .vz-spark{margin-top:0;width:100%;height:100%;min-height:28px}' +
      '.vz-spark-ax{display:grid;grid-template-columns:auto minmax(0,1fr);' +
        'grid-template-rows:auto minmax(0,1fr) auto auto;gap:0 6px;' +
        'width:100%;height:100%;min-width:0;min-height:0;box-sizing:border-box;padding:2px 4px 0}' +
      ...
      '.vz-spark-ax .vz-yunit{grid-column:1;grid-row:1;font-size:8px;color:var(--tf,#64748b);line-height:1;' +
        'white-space:nowrap;align-self:end;padding-bottom:2px;font-weight:700}' +
      '.vz-spark-ax .vz-ylabs{grid-column:1;grid-row:2;display:flex;flex-direction:column;justify-content:space-between;' +
        'align-items:flex-end;font-size:8px;color:var(--tlo);line-height:1.15;font-variant-numeric:tabular-nums;padding:2px 0;gap:2px;' +
        'min-width:2.8em;overflow:visible}' +
      '.vz-spark-ax .vz-plot{grid-column:2;grid-row:1 / span 2;min-width:0;min-height:0;position:relative;overflow:visible;' +
        'border-left:1px solid rgba(148,163,184,.4);border-bottom:1px solid rgba(148,163,184,.4)}' +
      ...
      '.vz-spark-ax .vz-plot svg{display:block;width:100%;height:100%;min-width:0;min-height:0;' +
        'max-width:100%;max-height:100%;margin:0;overflow:visible}' +
      /* 高低點數值標籤：半透明底＋避讓，不再只剩圓點；compact 也顯示 */
      '.vz-pt{position:absolute;font-size:9px;font-weight:800;line-height:1.15;' +
        'pointer-events:none;white-space:nowrap;font-variant-numeric:tabular-nums;z-index:3;' +
        'padding:1px 5px;border-radius:3px;letter-spacing:-0.2px;' +
        'background:rgba(6,10,18,.9);border:1px solid rgba(148,163,184,.38);' +
        'box-shadow:0 1px 4px rgba(0,0,0,.5);transform:translate(-50%,-135%)}' +
      '.vz-pt.hi{color:var(--thi,#F1F5FA);border-color:rgba(248,113,113,.5)}' +
      '.vz-pt.lo{color:#cbd5e1;border-color:rgba(148,163,184,.45);transform:translate(-50%,42%)}' +
      '.vz-pt.hi.flip{transform:translate(-50%,42%)}' +
      '.vz-pt.lo.flip{transform:translate(-50%,-135%)}' +
      '.vz-spark-ax.vz-compact .vz-pt{font-size:8px;padding:1px 4px}' +
      '.vz-spark-ax .vz-xlabs{grid-column:2;grid-row:3;display:flex;justify-content:space-between;' +
        'font-size:8px;color:var(--tlo);line-height:1.2;font-variant-numeric:tabular-nums;padding-top:3px}' +
      '.vz-spark-ax .vz-xunit{grid-column:1 / span 2;grid-row:4;font-size:8px;color:var(--tf,#64748b);line-height:1.2;' +
   ...
   *  opts.fill !== false → 面積漸層；opts.marks !== false → 高低點標記／標籤
   */
  function sparkLine(vals, opts) {
    ...
    var span = (hi - lo) || 1;
    var mid = (hi + lo) / 2;
    var w = opts.w || 240, h = opts.h || 36;
    var wantFill = opts.fill !== false;
    var wantMarks = opts.marks !== false;
    var compact = !!opts.compact;
    /* 有高低標時略增內距，讓數字不壓在線上 */
    var pad = opts.pad != null ? opts.pad : (wantMarks ? (compact ? 9 : 8) : 4);
    var strokeW = opts.strokeWidth != null ? opts.strokeWidth : 2.75;
    var n = arr.length;
    var xy = arr.map(function (c, i) {
      var x = pad + (i / (n - 1)) * (w - pad * 2);
      var y = pad + (1 - (c - lo) / span) * (h - pad * 2);
      return { x: x, y: y, v: c, i: i };
    });
    ...
    var gid = 'vzsg' + Math.abs((arr[0] * 1000 + n * 17 + hi * 3) | 0).toString(36) +
      (arr[n - 1] | 0).toString(36);
    var area = '';
    if (wantFill) {
      var yBase = (h - pad).toFixed(1);
      var areaPts = pad.toFixed(1) + ',' + yBase + ' ' + pts + ' ' +
        (w - pad).toFixed(1) + ',' + yBase;
      area =
    ...
    var iHi = 0, iLo = 0;
    for (var i = 1; i < n; i++) {
      if (arr[i] >= arr[iHi]) iHi = i;
      if (arr[i] <= arr[iLo]) iLo = i;
    }
    var markSvg = '';
    if (wantMarks) {
      markSvg =
        '<circle cx="' + xy[iHi].x.toFixed(1) + '" cy="' + xy[iHi].y.toFixed(1) +
          '" r="2.2" fill="' + stroke + '" stroke="rgba(6,10,18,.9)" stroke-width="1"/>' +
        '<circle cx="' + xy[iLo].x.toFixed(1) + '" cy="' + xy[iLo].y.toFixed(1) +
          '" r="2.2" fill="#94a3b8" stroke="rgba(6,10,18,.9)" stroke-width="1"/>';
    }
    var svg = '<svg class="vz-spark" viewBox="0 0 ' + w + ' ' + h +
      '" preserveAspectRatio="none" overflow="visible">' +
      area + midLine + zeroLine +
      '<polyline fill="none" stroke="' + stroke + '" stroke-width="' + strokeW +
      '" stroke-linecap="round" stroke-linejoin="round" vector-effect="non-scaling-stroke" points="' +
      pts + '"/>' + markSvg + '</svg>';

    /* 高低標：貼點旁顯示數值；靠邊側翻、過近左右錯開，避免互蓋或擋線 */
    function _peakLabelsHtml() {
      if (!wantMarks) return '';
      var leftPct = function (idx) { return (xy[idx].x / w) * 100; };
      var topPct = function (idx) { return (xy[idx].y / h) * 100; };
      var hiL = leftPct(iHi);
      var loL = leftPct(iLo);
      var hiT = topPct(iHi);
      var loT = topPct(iLo);
      var hiFlip = hiT < 18;   /* 高點太靠上 → 標籤改放下方 */
      var loFlip = loT > 82;   /* 低點太靠下 → 標籤改放上方 */
      if (iHi === iLo) {
        hiL = Math.max(8, hiL - 7);
        loL = Math.min(92, loL + 7);
        loFlip = !hiFlip;
      } else if (Math.abs(hiL - loL) < 14) {
        if (hiL <= loL) {
          hiL = Math.max(6, hiL - 5);
          loL = Math.min(94, loL + 5);
        } else {
          loL = Math.max(6, loL - 5);
          hiL = Math.min(94, hiL + 5);
        }
      }
      hiL = Math.max(5, Math.min(95, hiL));
      loL = Math.max(5, Math.min(95, loL));
      return '<span class="vz-pt hi' + (hiFlip ? ' flip' : '') +
        '" style="left:' + hiL.toFixed(1) + '%;top:' + hiT.toFixed(1) + '%">' +
        esc(_fmtAxisY(hi, opts)) + '</span>' +
        '<span class="vz-pt lo' + (loFlip ? ' flip' : '') +
        '" style="left:' + loL.toFixed(1) + '%;top:' + loT.toFixed(1) + '%">' +
        esc(_fmtAxisY(lo, opts)) + '</span>';
    }

    var peakHtml = _peakLabelsHtml();
    var xUnit = opts.xUnit != null ? String(opts.xUnit) : '';
    var yUnit = opts.yUnit != null ? String(opts.yUnit) : '';
    if (!xUnit && !yUnit && !opts.axes) {
      if (!peakHtml) return svg;
      return '<div class="vz-spark-wrap" title="高 ' + esc(_fmtAxisY(hi, opts)) +
        ' / 低 ' + esc(_fmtAxisY(lo, opts)) + '">' + svg + peakHtml + '</div>';
    }
    var xLbl = xUnit || '日';
    var yLbl = yUnit || '';
    ...
    return '<div class="vz-spark-ax' + (compact ? ' vz-compact' : '') + '" title="' +
      ...
      '<div class="vz-plot">' + svg + peakHtml + '</div>' +
